// assemble-manuel.js — assemble le manuel complet Physique-Chimie T8 J-Learn
...
const U1 = require("./data-unite1"); // S1..S9
...
const SP = {};
SPX.seances.forEach(s => { SP["S" + s.numero] = s; });

const ROOT = path.join(__dirname, "..");

// ---------- plan ----------
const unite1Seances = [...U1.seances, SP.S10, SP.S11];
...
const plan = [
  ...
  },
  {
    ...
    titre: "UNITÉ II — L'électricité",
    surtitre: "UNITÉ II", sousTitre: "L'ÉLECTRICITÉ",
  ...
  },
  {
    ...
    titre: "UNITÉ III — Mouvement et force",
    surtitre: "UNITÉ III", sousTitre: "MOUVEMENT ET FORCE",
  ...
  },
  {
    ...
    titre: "UNITÉ IV — Énergie, lumière et son",
    surtitre: "UNITÉ IV", sousTitre: "ÉNERGIE, LUMIÈRE ET SON",
...
for (const u of plan) {
  if (u.unite !== "EB") {
    ...
    children.push(B.p("Tout le programme : Matière • Électricité • Mouvement et force • Énergie, lumière et son", { bold: true, size: 26, align: AlignmentType.CENTER, spacingAfter: 200 }));
    children.push(B.p("Avant de commencer : relis tes fiches d'auto-évaluation, vérifie ton tableau de bord, prépare ta règle et ta calculatrice. Bon courage !", { italics: true, size: 22, align: AlignmentType.CENTER, spacingAfter: 200 }));
...
B.Packer.toBuffer(doc).then(buf => {
  ...
  const out = path.join(outDir, "Manuel_Physique-Chimie_T8_JLearn_V1.docx");
