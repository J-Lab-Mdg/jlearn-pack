#!/usr/bin/env python3
# ============================================================
# audit.py — contrôle du docx produit (Manuel SES T11, série L)
...
DOCX = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    os.path.dirname(__file__), "..", "output", "Manuel_SES_T11_L_JLearn.docx")
...
nums = sorted(int(x) for x in re.findall(r"SÉANCE (\d+) / 35", txt))
chk("Séances « n / 35 »", len(nums))
