// ============================================================
// assemble.js — assemblage final du manuel SVT T7 (5e)
// ============================================================
const fs = require("fs");
...
const UNITS = [U1, U2, U3, U4, U5].map((u) => {
...
function coverPage() {
  return [
    ...
    B.p("CLASSE DE T7 (5e)", { bold: true, size: 30, align: AlignmentType.CENTER, spacingAfter: 200 }),
...
function avantPropos() {
  ...
  out.push(B.p("Les séances sont regroupées en cinq unités, une par thématique du programme. Chaque unité se termine par une séance de révision et un « Sujet d'examen 5e » corrigé, avec barème.", { size: 21, spacingAfter: 120 }));
...
function tableauDeBord(u, startNum) {
  ...
  out.push(B.p("Résultat d'apprentissage spécifique (PE T7) : " + u.ras, { size: 20, spacingAfter: 60 }));
...
function evalExamenAnnexe() {
  ...
  out.push(B.p("Les sujets d'examen 5e de chaque unité sont regroupés ci-dessous pour préparer les évaluations.", { size: 20, spacingAfter: 80 }));
  let n = 0;
  UNITS.forEach((u) => {
    n += u.topics.length + 2;
