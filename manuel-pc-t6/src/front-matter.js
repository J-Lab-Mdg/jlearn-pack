...
function buildBookcover(rootDir) {
  ...
  const p = path.join(rootDir, "images/bookcover_PC_T6_a4.png");
...
function buildCouverture(rootDir) {
  ...
  out.push(B.p("Classe de T6", { bold: true, size: 30, align: AlignmentType.CENTER, spacingAfter: 300 }));
...
function buildAvantPropos() {
  ...
  [
    "Ce manuel de Sciences Physiques pour la classe de T6 a été conçu conformément au programme d'études officiel du Ministère de l'Éducation Nationale de Madagascar. Il couvre l'intégralité du programme de l'année en trente-trois séances, complétées par un examen blanc de fin d'année.",
    ...
    "Une attention particulière a été portée au contexte malgache : les exemples, les anecdotes et les expériences s'appuient sur la vie quotidienne des élèves — la marmite de riz, le fatapera, le chemin de fer Fianarantsoa-Côte Est, les brises côtières ou les jarres en terre cuite. Chaque séance propose en outre une rubrique « Expérience à la maison » réalisable avec du matériel simple, un encadré « Le savais-tu ? » pour éveiller la curiosité, et un tableau de matériel de substitution qui remplace le matériel de laboratoire par des équivalents locaux faciles à trouver.",
    "Trois sujets d'examen d'unité et un examen blanc final, tous corrigés et barémés, permettent d'évaluer régulièrement les acquis. Le tableau de bord placé au début du manuel aide l'apprenant à suivre sa progression séance après séance.",
    "Nous souhaitons que ce manuel soit un compagnon fidèle de l'enseignant comme de l'apprenant, et qu'il contribue à faire aimer les sciences physiques à tous les élèves de T6.",
