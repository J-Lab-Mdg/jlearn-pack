# -*- coding: utf-8 -*-
"""Génère SVT-9e-Bimestre3-Complet.docx (Traitement C, python-docx).
...
from data_b3a import SEANCES_A
from data_b3b import SEANCES_B, REVISION_B3, EXAMEN_B3
...
OUT = os.path.join(ROOT, "svt-9e", "output", "SVT-9e-Bimestre3-Complet.docx")
...
para(doc, "Classe de 9e — Bimestre 3", 16, NOIR, True, False, C, space_after=12)
...
para(doc, "Livraison 3 sur 5. La numérotation des séances (1 à 12) est provisoire : elle deviendra continue dans le manuel complet de fin d'année.", 11, NOIR, False, True, C)
page_break(doc)

# Avant-propos
para(doc, "Avant-propos", 16, NOIR, True, False, None, bookmark="bm_avantpropos")
para(doc, "Ce manuel de Sciences de la Vie et de la Terre pour la classe de 9e suit le programme officiel malgache. Ce troisième fascicule couvre le Bimestre 3 : le fonctionnement de l'œil, la protection et l'hygiène des yeux, l'oreille et la langue.")
...
sommaire_link(doc, "bm_tableau", "Tableau de bord du Bimestre 3")
for s in SEANCES:
    sommaire_link(doc, f"bm_seance{s['n']}", f"Séance {s['n']} : {s['titre']}")
sommaire_link(doc, "bm_revision", "Révision — Bimestre 3")
sommaire_link(doc, "bm_examen", "Sujet d'examen 9e — Bimestre 3")

# Tableau de bord
page_break(doc)
para(doc, "Tableau de bord du Bimestre 3", 16, NOIR, True, False, None, bookmark="bm_tableau")
...
# ---------------- révision ----------------
page_break(doc)
para(doc, REVISION_B3["titre"], 16, ROUGE, True, False, None, bookmark="bm_revision")
para(doc, "Je relis le tableau, puis je réponds aux questions.")
tr = doc.add_table(rows=1 + len(REVISION_B3["tableau"]), cols=2, style="Table Grid")
...
for i, (a, b) in enumerate(REVISION_B3["tableau"], start=1):
...
for i, (q, _a) in enumerate(REVISION_B3["questions"], start=1):
    para(doc, f"{i}. {q}")
para(doc, "Réponses attendues (R.A.)", 12, VERT, True)
for i, (_q, a) in enumerate(REVISION_B3["questions"], start=1):
    para(doc, f"{i}. {a}", kw_color=None)

# ---------------- examen ----------------
page_break(doc)
para(doc, EXAMEN_B3["titre"], 16, ROUGE, True, False, None, bookmark="bm_examen")
para(doc, f"Durée : {EXAMEN_B3['duree']} — Total : ... / {EXAMEN_B3['total']}", 11, NOIR, True)
for i, e in enumerate(EXAMEN_B3["parties"], start=1):
...
for i, e in enumerate(EXAMEN_B3["parties"], start=1):
    ...
    for c in e["cor"]:
        para(doc, c, kw_color=BORDEAUX)
para(doc, f"Total : ... / {EXAMEN_B3['total']}", 11, NOIR, True)
