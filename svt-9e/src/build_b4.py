# -*- coding: utf-8 -*-
"""Génère SVT-9e-Bimestre4-Complet.docx (Traitement C, python-docx).
...
from data_b4a import SEANCES_A
from data_b4b import SEANCES_B, REVISION_B4, EXAMEN_B4
...
OUT = os.path.join(ROOT, "svt-9e", "output", "SVT-9e-Bimestre4-Complet.docx")
...
para(doc, "Classe de 9e — Bimestre 4", 16, NOIR, True, False, C, space_after=12)
para(doc, "Programme officiel malgache — 9 séances", 12, NOIR, False, False, C, space_after=24)
para(doc, "Chaque séance : fiche de préparation + leçon illustrée + exercices corrigés.", 11, NOIR, False, True, C)
para(doc, "Livraison 4 sur 5. La numérotation des séances (1 à 9) est provisoire : elle deviendra continue dans le manuel complet de fin d'année.", 11, NOIR, False, True, C)
page_break(doc)

# Avant-propos
para(doc, "Avant-propos", 16, NOIR, True, False, None, bookmark="bm_avantpropos")
para(doc, "Ce manuel de Sciences de la Vie et de la Terre pour la classe de 9e suit le programme officiel malgache. Ce quatrième fascicule couvre le Bimestre 4 : la santé et la sécurité du membre supérieur, les membres inférieurs et l'appareil respiratoire.")
...
sommaire_link(doc, "bm_tableau", "Tableau de bord du Bimestre 4")
for s in SEANCES:
    sommaire_link(doc, f"bm_seance{s['n']}", f"Séance {s['n']} : {s['titre']}")
sommaire_link(doc, "bm_revision", "Révision — Bimestre 4")
sommaire_link(doc, "bm_examen", "Sujet d'examen 9e — Bimestre 4")

# Tableau de bord
page_break(doc)
para(doc, "Tableau de bord du Bimestre 4", 16, NOIR, True, False, None, bookmark="bm_tableau")
tb = doc.add_table(rows=6, cols=2, style="Table Grid")
...
lignes = [("Le membre supérieur en mouvement", "Séances 1 à 3"), ("Les membres inférieurs", "Séances 4 à 6"),
          ("L'appareil respiratoire", "Séances 7 à 9"),
          ("Révision — Bimestre 4", "Après la séance 9"), ("Sujet d'examen 9e — Bimestre 4", "Après la révision")]
...
# ---------------- révision ----------------
page_break(doc)
para(doc, REVISION_B4["titre"], 16, ROUGE, True, False, None, bookmark="bm_revision")
para(doc, "Je relis le tableau, puis je réponds aux questions.")
tr = doc.add_table(rows=1 + len(REVISION_B4["tableau"]), cols=2, style="Table Grid")
...
for i, (a, b) in enumerate(REVISION_B4["tableau"], start=1):
...
for i, (q, _a) in enumerate(REVISION_B4["questions"], start=1):
    para(doc, f"{i}. {q}")
para(doc, "Réponses attendues (R.A.)", 12, VERT, True)
for i, (_q, a) in enumerate(REVISION_B4["questions"], start=1):
    para(doc, f"{i}. {a}", kw_color=None)

# ---------------- examen ----------------
page_break(doc)
para(doc, EXAMEN_B4["titre"], 16, ROUGE, True, False, None, bookmark="bm_examen")
para(doc, f"Durée : {EXAMEN_B4['duree']} — Total : ... / {EXAMEN_B4['total']}", 11, NOIR, True)
for i, e in enumerate(EXAMEN_B4["parties"], start=1):
...
for i, e in enumerate(EXAMEN_B4["parties"], start=1):
    ...
    for c in e["cor"]:
        para(doc, c, kw_color=BORDEAUX)
para(doc, f"Total : ... / {EXAMEN_B4['total']}", 11, NOIR, True)
