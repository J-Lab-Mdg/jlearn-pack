// ============================================================
// assemble-bloc1.js — BLOC 1 : Thème 1 « Les aliments » (V2)
// Manuel Français 9e (T3/CE) — Collection J-Learn — skill v18
...
const TOTAL = 186; // numérotation continue sur tout le manuel
const OUT = path.join(__dirname, "..", "output", "Manuel_Francais_9e_V2_BLOC1.docx");
const { COLORS } = B;
...
function avantPropos() {
  ...
  o.push(t(["Ce manuel est construit à partir de ", { t: "deux programmes officiels utilisés conjointement", b: true }, " : la Répartition Annuelle du Programme d'Études (RAPE) de la classe de T3, année scolaire 2025-2026, et le programme d'études antérieur. Les thèmes issus d'un seul des deux programmes sont signalés dans le sommaire et en tête de thème. Cette double référence garantit qu'aucune notion n'est perdue."]));
...
function sommaire() {
  const o = [];
  o.push(B.pageBreakPara());
  o.push(B.bookmarkPara("sommaire", [{ t: "SOMMAIRE — THÈME 1 : LES ALIMENTS", b: true, color: COLORS.rouge }], { base: { size: 28 } }));
  theme1.seances.forEach((s) => o.push(B.sommaireLine("Séance " + s.n + " — " + s.titre + " (" + s.sd + ")", "seance" + s.n, 0)));
  o.push(B.sommaireLine("Annexes du thème 1 — conjugaison, grammaire, orthographe, vocabulaire", "theme1annexe", 0));
  return o;
}
...
function seance(s) {
  ...
  o.push(B.metaTable({
    discipline: "Français", sousDiscipline: s.sd, theme: theme1.nom,
...
function main() {
  ...
  theme1.seances.forEach((s) => children.push(...seance(s)));
  children.push(...buildAnnexe(annexe1));
  children.push(B.pageBreakPara());
  children.push(B.p([{ t: "FIN DU BLOC 1 (THÈME 1 — SÉANCES 1 À 3). À suivre : Thème 2 — La famille élargie.", b: true, color: COLORS.rouge }], { base: { size: 22 }, align: "center" }));
