...
const NOTE_METHODE = "Méthode facultative : si une carte murale, un manuel illustré, un texte d'époque ou des témoignages recueillis par enquête sont disponibles, les faire observer et commenter par les élèves à la place du document recopié au tableau (voir Mode d'emploi).";
...
function buildFiche(s, total) {
  ...
  const rows = [
    ...
    B.stepRow({
      ...
      enseignant: [`Aujourd'hui nous allons apprendre : « ${s.titre} ». Après cette séance, vous serez capables ${B.de(s.objectif)}.`],
  ...
  return [
    ...
    B.metaTable({
      discipline: "Histoire", theme: s.theme, titre: s.titre, objectif: s.objectif,
