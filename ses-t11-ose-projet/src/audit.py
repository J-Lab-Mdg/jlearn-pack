#!/usr/bin/env python3
# ============================================================
# audit.py — contrôle du docx produit (Manuel SES T11, série OSE)
...
DOCX = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
    os.path.dirname(__file__), "..", "output", "Manuel_SES_T11_OSE_JLearn.docx")
...
nums = sorted(int(x) for x in re.findall(r"SÉANCE (\d+) / 70", txt))
chk("Séances « n / 70 »", len(nums))
